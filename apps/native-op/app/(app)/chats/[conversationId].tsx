...
type ListItem =
  ...
  | { kind: "msg"; id: string; from: "them" | "me"; body: string; time: string };
...
export default function OperatorChatThreadScreen() {
  ...
  const tenantName =
    typeof params.tenantName === "string" && params.tenantName.length > 0
      ? params.tenantName
      : "Tenant";
  const propertyName =
    typeof params.propertyName === "string" && params.propertyName.length > 0
      ? params.propertyName
      : "";

  const [input, setInput] = useState("");
  const [sending, setSending] = useState(false);
  // null = never loaded yet (show spinner), Message[] = loaded (may be empty)
  ...
  }, [convex, conversationId, fetchMessages]);
  ...
  const listData = useMemo((): ListItem[] => {
    ...
    for (const msg of msgs) {
      ...
      items.push({
        kind: "msg",
        id: msg._id,
        from: msg.senderRole === "operator" ? "me" : "them",
        body: msg.body,
  ...
  const send = async () => {
    ...
    setInput("");
    try {
      await (convex as any).mutation("chats:sendMessageByConversation", {
        conversationId,
        body: trimmed,
      });
...
const s = StyleSheet.create({
  ...
  headerMeta: { flex: 1, marginHorizontal: 10, alignItems: "center" },
  headerTitle: {
    fontSize: 17,
    fontWeight: "800",
    color: colors.navy,
